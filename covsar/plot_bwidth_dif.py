...
def main():
    ...
    pixel_paths = glob.glob(data_root + '/imnavtest/p_*/')
    pixel_paths = [
    ...
    for p in range(len(pixel_paths)):
        ...
        dif_paths = glob.glob(os.path.join(
            path, 'dispDiff_*.np.npy'))

        dif_paths = sorted(dif_paths)

        bws = np.array([int(dif_path.split('_')[-1][0])
                        for dif_path in dif_paths])

        print(bws)
        max_bws = np.max(bws)

        cmap_lines = plt.get_cmap('gist_heat_r')
        for dif_path in dif_paths:
            differences = np.load(dif_path)

            bw = int(dif_path.split('_')[-1][0])
            ...
            axes[1, p].plot(days, differences, linewidth=2,
                            color=cmap_lines(int(100 * bw / max_bws)), label=bw)
            axes[1, p].legend(loc='best')
    ...
    plt.savefig('/Users/rbiessel/Documents/scatter.png', dpi=300)
    plt.show()
