...
def main():
    ...
    forward_model = SMForward(
        imag_slope=1/10, r_A=0.1, r_B=0.1, r_C=4)
    ...
    idif12_dub = forward_model.dubois_I_ratio(m1, m2, log=True)
    idif23_dub = forward_model.dubois_I_ratio(m2, m3, log=True)
    idif13_dub = forward_model.dubois_I_ratio(m3, m1, log=True)

    plt.scatter(m1, forward_model.fresnels(m1))
    plt.show()

    plt.scatter(m1, forward_model.I_dezan(m1) + forward_model.fresnels(m1))
    plt.show()

    idif12_dezn = np.log10((forward_model.I_dezan(m1) + forward_model.fresnels(m1)) /
                           (forward_model.I_dezan(m2) + forward_model.fresnels(m2)))

    sm_dif = m2 - m1
    # plt.scatter(m2 - m1, idif12_dub, s=5)
    plt.scatter(idif12_dezn, idif12_dub, s=5)
    plt.show()
    return
